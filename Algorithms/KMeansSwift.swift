...
public typealias KMVectors = Array<[Double]>
public typealias KMVector = [Double]
...
public let KMeans = KMeansSwift.shared
public class KMeansSwift {
    static let shared = KMeansSwift()
    fileprivate init() {}
    ...
    //MARK: --Public
    //Get from the same person
    public func getKMeanVectorSameName(cluster: Int, vectors: [Vector], completionHandler: @escaping ([Vector]) -> Void) {
        reset()
        for vector in vectors {
            addVector(vector.vector)
        }
        clusteringNumber = cluster
        clustering(500) { (success, centroids, clusters) -> () in
            if success {
    ...
    //check parameters
    public  func checkAllParameters() -> Bool {
    ...
    //add vectors
    public func addVector(_ newVector:KMVector) {
        vectors.append(newVector)
    }
    
    public func addVectors(_ newVectors:KMVectors) {
    ...
    //clustering
    public func clustering(_ numberOfExcutions:Int, completion:(_ success:Bool, _ centroids:KMVectors, _ clusters:[KMVectors])->()) {
    ...
    fileprivate func pickingInitialCentroidsRandomly() {
        guard clusteringNumber > 0 else { return }
